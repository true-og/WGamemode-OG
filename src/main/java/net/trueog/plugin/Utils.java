// This class is free and unencumbered software released into the public domain.
...
public class Utils {
    ...
    public static void trueogMessage(Player player, String message) {
        ...
        // Sends a message using legacy Bukkit color codes in any case with built-in
        // MiniPlaceholder expansion using the
        // TrueOG MiniPlaceholderAPI.
        player.sendMessage(serializerAnyCase(player, message));

    }

    // Converts all Bukkit color codes in the message to lowercase so the
    // case-sensitive MiniMessage legacy color code
    // translator works.
    public static TextComponent serializerAnyCase(Player player, String subject) {
        ...
        int index = 0;
        // Iterates through the subject String to find the positions of '&' characters
        ...
        for (int i = 0; i < subject.length(); i++) {
            ...
            if (subject.charAt(i) == '&') {
                ...
                if (isUpperBukkitCode(subject.charAt(i + 1))) {
                    ...
                    // one.
                    subject = replaceAtIndex(subject, (i + 1),
                            Character.toString(Character.toLowerCase(subject.charAt(i + 1))));
                ...
                // If the character after an ampersand is not a Bukkit color / formatting code,
                // do this...
        ...
        // Process the newly uniform modern color codes via MiniMessage, including
        // MiniPlaceholder expansion.
        TextComponent finalComponent = Utils.expandPlayerMiniPlaceholders(player, subject);
    ...
    // Replace MiniPlaceholders in a given String with their content from
    // MiniPlaceholderAPI.
    public static TextComponent expandPlayerMiniPlaceholders(Player player, String message) {

        try {

            // Get the MiniPlaceholder content that is relevant for the specific player who
            // is seeing the message.
            TagResolver placeholders = MiniPlaceholders.getAudiencePlaceholders(player);

            // Replace the MiniPlaceholders with the audience placeholder content and the
            // modern color codes with actual
            // colors.
        ...
        // If any MiniPlaceholders could not be expanded, do this...
            ...
            TextComponent formattedErrorMessage = Utils.serializerAnyCase(player,
                    ("\"Error formatting MiniPlaceholdersAPI message for player: \" + player.getName() + \"Error: \" + error.getMessage()"));
    ...
    // Sends an error message to the player if they don't have permission to run a
    // given command.
    public static void permissionsErrorMessage(Player player, String command, String permission) {
        ...
        trueogMessage(player, ("&cERROR: You do not have permission to run the /" + command
                + " command! &6Required permission: &e" + permission + "&6."));

    }

    // Sends an error message to the console if the command which was run is only
    ...
    public static void consoleUseErrorMessage(CommandSender sender, String command, String permission) {
        ...
        sender.sendMessage(("ERROR: The server console cannot execute the command: " + command
                + "! To run it in-game, ensure you have the permission: " + permission + "."));

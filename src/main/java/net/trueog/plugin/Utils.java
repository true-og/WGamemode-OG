...
public class Utils {
    ...
    public static void trueogMessage(Player player, String message) {
        ...
        // Sends a message using legacy Bukkit color codes in any case with built-in MiniPlaceholder expansion using the
        // TrueOG MiniPlaceholderAPI.
        player.sendMessage(serializerAnyCase(player, message));
    }

    // Converts all Bukkit color codes in the message to lowercase so the case-sensitive MiniMessage legacy color code
    // translator works.
        ...
        for (int i = 0; i < subject.length(); i++) {
            ...
            if (subject.charAt(i) == '&') {
                ...
                if (isUpperBukkitCode(subject.charAt(i + 1))) {
                    ...
                    subject = replaceAtIndex(
                            subject, (i + 1), Character.toString(Character.toLowerCase(subject.charAt(i + 1))));
                }
    ...
    public static TextComponent expandPlayerMiniPlaceholders(Player player, String message) {
        ...
        try {
            ...
            // Replace the MiniPlaceholders with the audience placeholder content and the modern color codes with actual
            // colors.
        ...
        // If any MiniPlaceholders could not be expanded, do this...
            ...
            TextComponent formattedErrorMessage = Utils.serializerAnyCase(
                    player,
    ...
    public static void permissionsErrorMessage(Player player, String command, String permission) {
        ...
        trueogMessage(
                player,
                ("&cERROR: You do not have permission to run the /" + command + " command! &6Required permission: &e"
                        + permission + "&6."));
    }
